<template>
  <div class="visitorOrder common-css">
    <div class="wrapper">
      <!--    导航栏-->
      ...
      <el-row class="footer" type="flex">
        <div class="left">
          订单总金额:
          <span class="red1">{{visitorOrderList.total}}</span>
        </div>
        <div class="right">
          <el-pagination
            :layout=page
            background
            :page-size="visitorOrderList.size"
            @current-change="changeClick"
            :total="visitorOrderList.total">
          </el-pagination>
        </div>
      </el-row>
    </div>
...
export default {
  ...
  mounted () {
    this.$store.dispatch('getAllVisitorOrder')
    this._initHeight()
  },
  created () {
